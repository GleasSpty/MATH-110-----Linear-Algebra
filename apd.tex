...
Before we get started, we need a couple of facts.
...
\begin{dfn}{$\K$-algebra homomorphism}{KAlgebraHomomorphism}
	Let $A$ and $B$ be $\K$-modules and let $\phi \colon A\rightarrow B$ be a function.  Then, $\phi$ is a \term{$\K$-algebra homomorphism} iff it both a ring homomorphism and $\K$-$\K$-bilinear.
\end{dfn}
...
\begin{cnv}{Two-sided}{TwoSided}
	In what follows, many definitions have left and right versions (because of the possible commutativity).  We then have a corresponding \term{two-sided}\index{Two-sided} version:  ABC is two-sided XYZ iff ABC is left XYZ and right XYZ.
...
\begin{dfn}{Algebraically-closed}{AlgebraicallyClosed}
	Let $\K$ be a ring.
	\begin{enumerate}
		\item $\K$ is \term{right algebraically-closed}\index{Right algebraically-closed} iff every nonconstant $p\in \K [x]$ has a right root in $\K$.
		\item $\K$ is \term{left algebraically-closed}\index{Left algebraically-closed} iff every nonconstant $p\in [x]\K$ has a left root in $\K$.
		\item $\K$ is \term{algebraically-closed}\index{Algebraically-closed} iff every nonconstant $\K [x]\ni p\in [x]\K$ has a root.
	\end{enumerate}
	\begin{rmk}
...
\begin{dfn}{Integrally-closed}{IntegrallyClosed}
	Let $\K$ be a ring.
	\begin{enumerate}
		\item $\K$ is \term{right integrally-closed}\index{Right integrally-closed} iff every nonconstant monic $p\in \K [x]$ has a right root in $\K$.
		\item $\K$ is \term{left algebraically-closed}\index{Left algebraically-closed} iff every nonconstant monic $p\in [x]\K$ has a left root in $\K$.
	\end{enumerate}
	\begin{rmk}
...
\begin{dfn}{Algebraic element}{AlgebraicElement}
	Let $\L$ be a $\K$-algebra and let $\alpha \in \L$.  Then, $\alpha$ is \term{right (resp.~left) algebraic}\index{Algebraic element} over $\K$ iff there is a nonconstant polynomial $p\in \K [x]$ such that $\alpha$ is a right (resp.~left) root of $p$.
	
	$\alpha$ is \term{right (resp.~left) transcendental}\index{Transcendental element} over $\K$ iff it is not right (resp.~left) algebraic over $\K$.
	\begin{rmk}
		Simply \term{algebraic} (or \term{two-sided algebraic}) means that $\alpha$ is a (two-sided) root of some nonconstant polynomial.
	\end{rmk}
\end{dfn}
\begin{dfn}{Integral element}{IntegralElement}
	Let $\L$ be a $\K$-algebra and let $\alpha \in \L$.  Then, $\alpha$ is \term{right (resp.~left) integral}\index{Integral element} over $\K$ iff there is a nonconstant monic polynomial $p\in \K [x]$ such that $\alpha$ is a right (resp.~left) root of $p$.
	\begin{rmk}
		Simply \term{integral} (or \term{two-sided integral}) means that $\alpha$ is a (two-sided) root of some nonconstant monic polynomial.
	\end{rmk}
\end{dfn}
\begin{exm}{}{}
...
\begin{dfn}{Algebraic $\K$-algebra}{AlgebraicKAlgebra}
	Let $\L$ be a $\K$-algebra.  Then, $\L$ is \term{right (resp.~left, resp.~two-sided) algebraic}\index{Algebraic $\K$-algebra} iff every element of $\L$ is right (resp.~left, resp.~two-sided) algebraic over $\K$.
	\begin{rmk}
		We may instead say that $\L$ is \term{algebraic} over $\K$.
	\end{rmk}
	\begin{rmk}
		If $\L$ is an extension of $\K$, then people will say that ``$L$ is a \term{right (resp.~left, resp.~two-sided) algebraic extension}\index{Algebraic extension} of $\K$'' instead of that ``$\L$ is a right (resp.~left, resp.~two-sided) algebraic $\K$-algebra''.
	\end{rmk}
\end{dfn}
\begin{dfn}{Integral $\K$-algebra}{AlgebraicKAlgebra}
	Let $\L$ be a $\K$-algebra.  Then, $\L$ is \term{right (resp.~left, resp.~two-sided) integral}\index{Integral $\K$-algebra} iff every element of $\L$ is right (resp.~left, resp.~two-sided) integral over $\K$.
	\begin{rmk}
...
\begin{dfn}{Algebraic-closure}{}
	Let $\L$ be a $\K$-algebra.  Then, $\L$ is an \term{right (resp.~left, resp.~two-sided) algebraic-closure}\index{Algebraic-closure} of $\K$ iff it is algebraically-closed and algebraic over $\K$.
\end{dfn}
\begin{dfn}{Integral-closure}{IntegralClosure}
	Let $\L$ be a $\K$-algebra.  Then, $\L$ is an \term{right (resp.~left, resp.~two-sided) algebraic-closure}\index{Integral-closure} of $\K$ iff it is algebraically-closed and algebraic over $\K$.
\end{dfn}
\begin{exm}{}{}
	\begin{enumerate}
...
\begin{prp}{Relative integral closure}{RelativeIntegralClosure}
	...
	\end{equation}
	is an integral $\K$-subalgebra of $A$, the \term{integral closure}\index{Relative integral closure} of $\K$ in $A$, which is integrally-closed if $A$ is.
	\begin{rmk}
...
\begin{thm}{Rings have algebraically-closed algebras}{thmC.2.25}
	...
	\begin{proof}
		...
		\Step{Define the algebraically-closed $\K$-algebra}
		...
		\Step{Check that $\K _{\infty}$ is indeed algebraically-closed}
...
\begin{thm}{Integral-closure}{AlgebraicClosure}
	Let $\K$ be a cring.  Then, there is a unique commutative integral-closure $\A _{\K}$\index[notation]{$\A _{\K}$} of $\K$, the \term{integral-closure}\index{Integral-closure} of $\K$, that has the property that, for any other integrally-closed commutative $\K$-algebra $A$, there is a unique $\K$-algebra homomorphism such that the following diagram commutes.
	\begin{equation}
		\begin{tikzcd}
...
\begin{dfn}{Perfect field}{PerfectField}
	Let $\F$ be a field.  Then, $\F$ is \term{perfect}\index{Perfect field} iff every irreducible polynomial $p\in \F [x]$ has distinct roots.
	...
	\end{rmk}
	\begin{rmk}
		$x_0\in \F$ is a root of $p$ iff $x-x_0$ divides $p$.  $p$ \term{has distinct roots}\index{Distinct roots} iff $(x-x_0)^2$ does \emph{not} divide $p$ for every root $x_0$.
